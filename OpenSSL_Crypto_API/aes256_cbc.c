...
int main(int argc, char *argv[]) {
    ...
    const char *encrypted_file = "aes256_encrypted.bin";
    const char *decrypted_file = "aes256_decrypted.txt";
    ...
    if (!encrypt_file(plaintext_file, encrypted_file, key, iv, EVP_aes_256_cbc())) {
    ...
    // Decryption
    clock_gettime(CLOCK_MONOTONIC, &start);
    if (!decrypt_file(encrypted_file, decrypted_file, key, iv, EVP_aes_256_cbc())) {
    ...
    printf("\nTIMING_RESULT:AES-256-CBC:%.6f:%.6f:%.6f\n", 
