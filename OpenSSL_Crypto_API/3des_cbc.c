...
#define KEY_LEN 24  // 192 bits (3 x 64-bit DES keys)
#define IV_LEN 8    // 64 bits

int main(int argc, char *argv[]) {
    const char *plaintext_file = "plaintext.txt";
    const char *encrypted_file = "3des_encrypted.bin";
    const char *decrypted_file = "3des_decrypted.txt";
    ...
    if (!encrypt_file(plaintext_file, encrypted_file, key, iv, EVP_des_ede3_cbc())) {
    ...
    // Decryption
    clock_gettime(CLOCK_MONOTONIC, &start);
    if (!decrypt_file(encrypted_file, decrypted_file, key, iv, EVP_des_ede3_cbc())) {
    ...
    printf("\nTIMING_RESULT:3DES-CBC:%.6f:%.6f:%.6f\n", 
