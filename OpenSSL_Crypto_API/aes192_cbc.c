...
#define KEY_LEN 24  // 192 bits
#define IV_LEN 16   // 128 bits

int main(int argc, char *argv[]) {
    const char *plaintext_file = "plaintext.txt";
    const char *encrypted_file = "aes192_encrypted.bin";
    const char *decrypted_file = "aes192_decrypted.txt";
    ...
    if (!encrypt_file(plaintext_file, encrypted_file, key, iv, EVP_aes_192_cbc())) {
    ...
    // Decryption
    clock_gettime(CLOCK_MONOTONIC, &start);
    if (!decrypt_file(encrypted_file, decrypted_file, key, iv, EVP_aes_192_cbc())) {
    ...
    printf("\nTIMING_RESULT:AES-192-CBC:%.6f:%.6f:%.6f\n", 
